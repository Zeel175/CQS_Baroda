...
namespace ITfoxtec.Identity.Saml2
{
    ...
    public class Saml2AuthnResponse : Saml2Response
    {
        ...
        protected internal override void Read(string xml, bool validateXmlSignature = false)
        {
            ...
            if (Status == Schemas.Saml2StatusCodes.Success)
            {
                var assertionElement = GetDecriptedAssertion(XmlDocument.DocumentElement, Config.SigningCertificate.PrivateKey); //GetAssertionElement();
        ...
        private XmlElement GetAssertionElementReference()
        {
            ...
            var assertionElements = XmlDocument.DocumentElement.SelectNodes($"//*[local-name()='{Schemas.Saml2Constants.Message.EncryptedAssertion}']");

            var xaa = XmlDocument.DocumentElement.SelectNodes($"//*[local='{Schemas.Saml2Constants.Message.saml2EncryptedAssertion}']");

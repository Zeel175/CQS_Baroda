...
(function ($) {
    ...
    $.fn.fSelect = function (options) {
        ...
        //function fReload(select, settings) {
        //    debugger;
        //    this.$select = $(select);
        //    this.settings = settings;
        //    this.reload();
    ...
    $(document).on('click', '.fs-option:not(.hidden, .disabled)', function (e) {
        ...
        if ($wrap.hasClass('multiple')) {
            ...
            if ($(this).text() == "Select All" || $(this).attr('data-value') == "0") {
                if ($(this).hasClass('selected')) {
                    $(this).parent().find('.fs-option').addClass('selected');
                }
                else {
                    //$('.fs-option').removeClass('selected');
                    $(this).parent().find('.fs-option').removeClass('selected');
                }
            }
    ...
    function openDropdown($wrap) {
        ...
        //debugger;
        // Reorder selected items to the top
        var $optionsContainer = $wrap.find('.fs-options');
        var $selectedOptions = $optionsContainer.find('.fs-option.selected');
        $selectedOptions.prependTo($optionsContainer);  // Move selected items to the top
        ...
        if ($wrap.find("select").hasClass("jugadClass")) {
            if ($wrap.parent().hasClass('auditeeClass')) {
                var leftD = $wrap.parent().offset().left - 185;
                $wrap.find('.fs-dropdown').css({ "left": `${parseInt(leftD)}px` })
            }
            else if ($wrap.parent().hasClass('auditorClass')) {
                var leftD = $wrap.parent().offset().left - 290;
                $wrap.find('.fs-dropdown').css({ "left": `${parseInt(leftD)}px` })
            }
            else if ($wrap.parent().parent().hasClass('ToUserClass')) {
                var leftD = $wrap.parent().offset().left - 290;
                $wrap.find('.fs-dropdown').css({ "left": `${parseInt(leftD)}px` })
            }
            else if ($wrap.parent().hasClass('ToUserClass')) {
                var leftD = $wrap.parent().offset().left - 290;
                $wrap.find('.fs-dropdown').css({ "left": `${parseInt(leftD)}px` })
            }
            else if ($wrap.parent().parent().hasClass('CCUserClass')) {
                var leftD = $wrap.parent().offset().left - 75;
                console.log($wrap.parent().offset().left);
                console.log(leftD);
                $wrap.find('.fs-dropdown').css({ "left": `${parseInt(leftD)}px` })
            }
            else {
    ...
    function closeDropdown($wrap) {
        ...
        if ($('.fs-wrap').find("select").hasClass("jugadClass")) {
            $('.fs-wrap').find(".fs-dropdown").css('left', '');
        }
